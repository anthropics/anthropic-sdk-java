...
internal class StructuredOutputsTest {
    ...
    fun schemaTest_mapHasNoNamedProperties() {
        ...
        assertThat(validator.toString())
            .startsWith(
                "JsonSchemaValidator{isValidationComplete=true, errors=[" +
    ...
    fun schemaTest_minimalListSchema() {
        ...
        //     }
        //
        // The reason for the missing `"type"` is that generic type information is erased for scopes
        // like local variables, but generic type information for fields is retained as part of the
        // class metadata. This is the expected behavior in Java, so this test expects the `"type"`
        // to be missing. That is acceptable for Anthropic schemas.
        //
        ...
        // Confirm that the schema contains `"items" : { }`, so the test is valid.
        assertThat(schema.get("items").isEmpty).isTrue
    }
    ...
    fun schemaTest_minimalEnumSchema2() {
        ...
        // other tests.
        assertThat(validator.toString())
            .isEqualTo("JsonSchemaValidator{isValidationComplete=true, errors=[]}")
    }
    ...
    @Test
    fun schemaTest_arrayTypeMissingItemsAllowed() {
        // `"items"` is optional for Anthropic. Just make sure there is no attempt at recursion
    ...
    @Test
    fun schemaTest_arrayTypeEmptyItemsAllowed() {
        // `"items"` is may be empty for Anthropic. Just make sure there is no attempt at recursion
        // that would result in an error. `List<Any>` is the same as using just `List` in Java. With
        // no generic type, there is no `"type"` to put into the `"items"` field's object.
        @Suppress("unused") class X(val l: List<Any>)
        ...
        assertThat(validator.isValid()).isTrue
        // Confirm that the schema contains `"items" : { }`, so the test is valid.
        assertThat(schema.get("properties").get("l").get("items").isEmpty).isTrue
    }
    ...
    fun schemaTest_arrayTypeWithWrongItemsType() {
        ...
        assertThat(validator.errors()).hasSize(1)
        assertThat(validator.errors()[0]).isEqualTo("#: 'items' field is not an object.")
    }
    ...
    fun schemaTest_badOptionalTypeNotArray() {
        ...
        // Only the size of the `errors()` list will be checked in other tests.
    ...
    fun schemaTest_unsupportedKeywords() {
        // Anthropic lists a set of keywords that are allowed and some that are not allowed. Check
        // that a selection of keywords that are not listed, or listed as not allowed, are
        // identified as not allowed. Check at the root schema and a sub-schema. There is no need to
        // match the keywords to their expected schema types or be concerned about the values of the
        // keyword fields, which makes testing easier. Supported keywords are tested elsewhere
        // (mostly when applied via annotations).
    ...
    fun schemaTest_anyOfInRootSchema() {
        // Anthropic has no restriction on the use of `"anyOf"` anywhere in the schema, including in
        // the root schema.
        schema =
            parseJson(
                """
    ...
    fun schemaTest_annotatedWithJsonPropertyRejectDefaultValue() {
        ...
        // as default property values are not supported in Anthropic JSON schemas. (The Victools
        // docs have examples of how to add support for these default values via annotations or
        // initial values, should support for default values be needed in the future.)
        @Suppress("unused")
        class X(
    ...
    @Test
    fun schemaTest_annotatedWithSchemaStringFormatSupported() {
        // NOTE: There is no validation of the `"format"` constraint _value_. This is done to make
        // maintenance easier if the specification changes in the future.
    ...
    @Test
    fun schemaTest_annotatedWithSchemaStringConstraintsUnsupported() {
        @Suppress("unused") class X(@get:Schema(pattern = "^[a-z]+$") val s: String)
        ...
        assertThat(validator.errors()).hasSize(1)
        assertThat(validator.errors()[0])
            .isEqualTo("#/properties/s: Use of 'pattern' is not supported here.")
    }

    @Test
    fun schemaTest_annotatedWithSchemaNumberConstraintsUnsupported() {
        @Suppress("unused")
        ...
        // Do not assume that the errors will appear in a reliable order. Use a set and `contains`.
        val expectedErrors =
            setOf(
                "#/properties/d: Use of 'minimum' is not supported here.",
                "#/properties/d: Use of 'maximum' is not supported here.",
                "#/properties/d: Use of 'multipleOf' is not supported here.",
            )

        assertThat(validator.errors()).hasSize(3)
        validator.errors().forEach { assertThat(expectedErrors.contains(it)).isTrue() }
    }

    @Test
    fun schemaTest_annotatedWithSchemaNumberConstraintsExclusiveUnsupported() {
        @Suppress("unused")
        class X(
            @get:Schema(
                multipleOf = 5.0,
        ...
        val expectedErrors =
            setOf(
                "#/properties/d: Use of 'exclusiveMinimum' is not supported here.",
                "#/properties/d: Use of 'exclusiveMaximum' is not supported here.",
                "#/properties/d: Use of 'multipleOf' is not supported here.",
            )
    ...
    @Test
    fun schemaTest_annotatedWithSchemaIntegerConstraintsUnsupported() {
        @Suppress("unused")
        ...
        val expectedErrors =
            setOf(
                "#/properties/i: Use of 'minimum' is not supported here.",
                "#/properties/i: Use of 'maximum' is not supported here.",
                "#/properties/i: Use of 'multipleOf' is not supported here.",
            )
    ...
    @Test
    fun schemaTest_annotatedWithSchemaIntegerConstraintsExclusiveUnsupported() {
        @Suppress("unused")
        class X(
            @get:Schema(
                multipleOf = 5.0,
        ...
        val expectedErrors =
            setOf(
                "#/properties/i: Use of 'exclusiveMinimum' is not supported here.",
                "#/properties/i: Use of 'exclusiveMaximum' is not supported here.",
                "#/properties/i: Use of 'multipleOf' is not supported here.",
            )
    ...
    @Test
    fun schemaTest_annotatedWithArraySchemaArrayMinItems0() {
        @Suppress("unused") class X(@get:ArraySchema(minItems = 0) val a: List<String>)
        ...
        assertThat(arrayProperty.get("minItems").asInt()).isEqualTo(0)
    }

    @Test
    fun schemaTest_annotatedWithArraySchemaArrayMinItems1() {
        // NOTE: There is no validation of the `"minItems"` constraint _value_, so only check that
        // the currently-supported values work and not that currently-unsupported values do not
        // work. This is done to make maintenance easier if the specification changes in the future.
        @Suppress("unused") class X(@get:ArraySchema(minItems = 1) val a: List<String>)
        ...
        assertThat(arrayProperty.get("minItems")).isNotNull
        assertThat(arrayProperty.get("minItems").isInt).isTrue
        assertThat(arrayProperty.get("minItems").asInt()).isEqualTo(1)
    }

    @Test
    fun schemaTest_annotatedWithArraySchemaArrayConstraintsUnsupported() {
        @Suppress("unused") class X(@get:ArraySchema(maxItems = 42) val a: List<String>)
        ...
        assertThat(validator.errors()).hasSize(1)
        assertThat(validator.errors()[0])
            .isEqualTo("#/properties/a: Use of 'maxItems' is not supported here.")
    }
    ...
    fun validatorBeforeValidation() {
        ...
        // tests.
        assertThat(validator.toString())
            .isEqualTo("JsonSchemaValidator{isValidationComplete=false, errors=[]}")
    }
    ...
    fun fromJsonSuccess() {
        ...
        val x = outputTypeFromJson("{\"s\" : \"hello\"}", X::class.java)
    ...
    @Test
    @Suppress("unused")
    fun outputFormatFromClassSuccessWithoutValidation() {
    ...
    @Test
    fun outputFormatFromClassSuccessWithValidation() {
        // `format` _is_ supported (its value is not checked).
        @Suppress("unused") class X(@get:Schema(format = "email") val s: String)
    ...
    @Test
    @Suppress("unused")
    fun outputFormatFromClassFailureWithValidation() {
        // A class that results in an invalid schema (`"pattern"` is not a supported keyword).
        @Suppress("unused") class X(@get:Schema(pattern = "unsupported") val s: String)
        ...
        assertThatThrownBy { outputFormatFromClass(X::class.java, JsonSchemaLocalValidation.YES) }
            ...
            .hasMessage(
                "Local validation failed for JSON schema derived from ${X::class.java}:\n" +
                    " - #/properties/s: Use of 'pattern' is not supported here."
            )
    }

    @Test
    @Suppress("unused")
    fun outputFormatFromClassFailureWithValidationDefault() {
        // A class that results in an invalid schema (`"pattern"` is not a supported keyword).
        @Suppress("unused") class X(@get:Schema(pattern = "unsupported") val s: String)
    ...
    fun extractFunctionInfoUsingClassNameAndNoDescription() {
        @Suppress("unused") class ParseXML(val s: String)

        val info = extractFunctionInfo(ParseXML::class.java, JsonSchemaLocalValidation.NO)

        assertThat(info.name).isEqualTo("parse_xml")
    ...
    @Test
    fun toolFromClassEnablesStrictAdherenceToSchema() {
        @Suppress("unused") @JsonClassDescription("Something about X") class X(val s: String)

        val tool = toolFromClass(X::class.java)

        // The "strict" flag _must_ be set to ensure that the model's output will _always_ conform
        // to the JSON schema.
        assertThat(tool.strict()).isPresent
        assertThat(tool.strict().get()).isTrue
        ...
        assertThat(tool.description().get()).isEqualTo("Something about X")

        // The `inputSchema()` accessor cannot be called successfully because of the way the field
    ...
    @Test
    @Suppress("unused")
    fun toolFromClassSuccessWithoutValidation() {
        // A class that results in an invalid schema (`"pattern"` is not a supported keyword).
        @Suppress("unused") class X(@get:Schema(pattern = "unsupported") val s: String)

        assertThatNoException().isThrownBy {
            toolFromClass(X::class.java, JsonSchemaLocalValidation.NO)
        }
    }

    @Test
    fun toolFromClassSuccessWithValidation() {
        @Suppress("unused") class X(@get:Schema(format = "email") val s: String)

        assertThatNoException().isThrownBy {
            toolFromClass(X::class.java, JsonSchemaLocalValidation.YES)
        }
    }

    @Test
    @Suppress("unused")
    fun toolFromClassFailureWithValidation() {
        // A class that results in an invalid schema (`"pattern"` is not a supported keyword).
        @Suppress("unused") class X(@get:Schema(pattern = "unsupported") val s: String)

        assertThatThrownBy { toolFromClass(X::class.java, JsonSchemaLocalValidation.YES) }
            .isExactlyInstanceOf(IllegalArgumentException::class.java)
            .hasMessage(
                "Local validation failed for JSON schema derived from ${X::class.java}:\n" +
                    " - #/properties/s: Use of 'pattern' is not supported here."
    ...
    @Test
    @Suppress("unused")
    fun toolFromClassFailureWithValidationDefault() {
        ...
        assertThatThrownBy { toolFromClass(X::class.java) }
            .isExactlyInstanceOf(IllegalArgumentException::class.java)
            .hasMessage(
                "Local validation failed for JSON schema derived from ${X::class.java}:\n" +
                    " - #/properties/s: Use of 'pattern' is not supported here."
    ...
    @Test
    fun toJsonString() {
        @Suppress("unused") class X(val s1: String, val s2: String)
        ...
        val twoStringObject = toJsonString(X("hello", "world"))
        ...
        assertThat(twoStringObject).isEqualTo("{\"s1\":\"hello\",\"s2\":\"world\"}")
    }

    @Test
    fun toSnakeCase() {
        assertThat("XMLParser".toSnakeCase()).isEqualTo("xml_parser")
        assertThat("XmlParser".toSnakeCase()).isEqualTo("xml_parser")
        assertThat("xmlParser".toSnakeCase()).isEqualTo("xml_parser")
        assertThat("NewXMLParser".toSnakeCase()).isEqualTo("new_xml_parser")
        assertThat("NewXmlParser".toSnakeCase()).isEqualTo("new_xml_parser")
        assertThat("ParseXML".toSnakeCase()).isEqualTo("parse_xml")
    }
}

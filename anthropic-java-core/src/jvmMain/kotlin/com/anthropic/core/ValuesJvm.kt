...
fun JsonValue.Companion.fromJsonNode(node: JsonNode): JsonValue = when (node.nodeType) {
    MISSING -> JsonMissing.of(); NULL -> JsonNull.of()
    BOOLEAN -> JsonBoolean.of(node.booleanValue()); NUMBER -> JsonNumber.of(node.numberValue())
    ...
    BINARY, POJO, null -> throw IllegalStateException("Unexpected JsonNode type: ${node.nodeType}")
}
...
// --- Optional wrappers (for public Java API backward compat) ---

fun <T : Any> JsonField<T>.getOptional(name: String): Optional<@UnsafeVariance T> =
    Optional.ofNullable(getNullable(name))

fun <T : Any> MultipartField<T>.filenameOptional(): Optional<String> = Optional.ofNullable(filename())

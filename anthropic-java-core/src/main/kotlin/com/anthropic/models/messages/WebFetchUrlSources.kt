...
private constructor(
    ...
    companion object {
        ...
        /** Returns a mutable builder for constructing an instance of [WebFetchUrlSources]. */
        @JvmStatic fun builder() = Builder()
    }

    /** A builder for [WebFetchUrlSources]. */
    class Builder internal constructor() {
        ...
        @JvmSynthetic
        internal fun from(webFetchUrlSources: WebFetchUrlSources) = apply {
            clientToolResults = webFetchUrlSources.clientToolResults
            serverToolResults = webFetchUrlSources.serverToolResults
            userInput = webFetchUrlSources.userInput
            additionalProperties = webFetchUrlSources.additionalProperties.toMutableMap()
        }
         ...
         * ```
         */
        fun onlyClientToolResults(tools: List<WebFetchUrlSourceToolReference>) =
            clientToolResults(WebFetchUrlSourceOnly.builder().tools(tools).build())

        /** Alias for calling [clientToolResults] with `ClientToolResults.ofExcept(except)`. */
        fun clientToolResults(except: WebFetchUrlSourceExcept) =
         ...
         * ```
         */
        fun exceptClientToolResults(tools: List<WebFetchUrlSourceToolReference>) =
            clientToolResults(WebFetchUrlSourceExcept.builder().tools(tools).build())
         ...
         * ```
         */
        fun onlyServerToolResults(tools: List<WebFetchUrlSourceToolReference>) =
            serverToolResults(WebFetchUrlSourceOnly.builder().tools(tools).build())

        /** Alias for calling [serverToolResults] with `ServerToolResults.ofExcept(except)`. */
        fun serverToolResults(except: WebFetchUrlSourceExcept) =
         ...
         * ```
         */
        fun exceptServerToolResults(tools: List<WebFetchUrlSourceToolReference>) =
            serverToolResults(WebFetchUrlSourceExcept.builder().tools(tools).build())
        ...
        fun userInput(all: WebFetchUrlSourceAll) = userInput(UserInput.ofAll(all))

        /** Alias for calling [userInput] with `UserInput.ofNone(none)`. */
        fun userInput(none: WebFetchUrlSourceNone) = userInput(UserInput.ofNone(none))
        ...
        /**
         * Returns an immutable instance of [WebFetchUrlSources].
         *
         * Further updates to this [Builder] will not mutate the returned instance.
         */
        fun build(): WebFetchUrlSources =
            WebFetchUrlSources(
                clientToolResults,
                serverToolResults,
                userInput,
     ...
     *   expected type.
     */
    fun validate(): WebFetchUrlSources = apply {
        if (validated) {
            return@apply
        }
    ...
    private constructor(
        private val all: WebFetchUrlSourceAll? = null,
        private val none: WebFetchUrlSourceNone? = null,
        private val only: WebFetchUrlSourceOnly? = null,
        private val except: WebFetchUrlSourceExcept? = null,
         ...
         * tool filter's source, or all user input.
         */
        fun asAll(): WebFetchUrlSourceAll = all.getOrThrow("all")

        /**
         * The ``url_sources`` variant under which a source contributes nothing: no result of the
         * tool filter's source, or no user input.
         */
        fun asNone(): WebFetchUrlSourceNone = none.getOrThrow("none")

        /** The tool filter variant under which only the named tools' results contribute. */
        fun asOnly(): WebFetchUrlSourceOnly = only.getOrThrow("only")

        /** The tool filter variant under which every result but the named tools' contributes. */
        fun asExcept(): WebFetchUrlSourceExcept = except.getOrThrow("except")
         ...
         *     @Override
         *     public Optional<String> visitAll(WebFetchUrlSourceAll all) {
        ...
        companion object {
             ...
             */
            @JvmStatic fun ofAll(all: WebFetchUrlSourceAll) = ClientToolResults(all = all)

            /**
             * The ``url_sources`` variant under which a source contributes nothing: no result of
             * the tool filter's source, or no user input.
             */
            @JvmStatic fun ofNone(none: WebFetchUrlSourceNone) = ClientToolResults(none = none)

            /** The tool filter variant under which only the named tools' results contribute. */
            @JvmStatic fun ofOnly(only: WebFetchUrlSourceOnly) = ClientToolResults(only = only)
             ...
             */
            @JvmStatic
            fun ofOnly(tools: List<WebFetchUrlSourceToolReference>) =
                ofOnly(WebFetchUrlSourceOnly.of(tools))

            /**
             * The tool filter variant under which every result but the named tools' contributes.
             */
            @JvmStatic
            fun ofExcept(except: WebFetchUrlSourceExcept) = ClientToolResults(except = except)
             ...
             */
            @JvmStatic
            fun ofExcept(tools: List<WebFetchUrlSourceToolReference>) =
                ofExcept(WebFetchUrlSourceExcept.of(tools))
        }
        ...
        interface Visitor<out T> {
             ...
             * the tool filter's source, or all user input.
             */
            fun visitAll(all: WebFetchUrlSourceAll): T

            /**
             * The ``url_sources`` variant under which a source contributes nothing: no result of
             * the tool filter's source, or no user input.
             */
            fun visitNone(none: WebFetchUrlSourceNone): T

            /** The tool filter variant under which only the named tools' results contribute. */
            fun visitOnly(only: WebFetchUrlSourceOnly): T

            /**
             * The tool filter variant under which every result but the named tools' contributes.
             */
            fun visitExcept(except: WebFetchUrlSourceExcept): T
        ...
        internal class Deserializer :
            ...
            override fun ObjectCodec.deserialize(node: JsonNode): ClientToolResults {
                ...
                when (type) {
                    "all" -> {
                        return tryDeserialize(node, jacksonTypeRef<WebFetchUrlSourceAll>())?.let {
                            ClientToolResults(all = it, _json = json)
                        } ?: ClientToolResults(_json = json)
                    }
                    "none" -> {
                        return tryDeserialize(node, jacksonTypeRef<WebFetchUrlSourceNone>())?.let {
                            ClientToolResults(none = it, _json = json)
                        } ?: ClientToolResults(_json = json)
                    }
                    "only" -> {
                        return tryDeserialize(node, jacksonTypeRef<WebFetchUrlSourceOnly>())?.let {
                            ClientToolResults(only = it, _json = json)
                        } ?: ClientToolResults(_json = json)
                    }
                    "except" -> {
                        return tryDeserialize(node, jacksonTypeRef<WebFetchUrlSourceExcept>())
    ...
    private constructor(
        ...
        companion object {
             ...
             * the tool filter's source, or all user input.
             */
            @JvmStatic fun ofAll(all: WebFetchUrlSourceAll) = ServerToolResults(all = all)

            /**
             * The ``url_sources`` variant under which a source contributes nothing: no result of
             * the tool filter's source, or no user input.
             */
            @JvmStatic fun ofNone(none: WebFetchUrlSourceNone) = ServerToolResults(none = none)

            /** The tool filter variant under which only the named tools' results contribute. */
            @JvmStatic fun ofOnly(only: WebFetchUrlSourceOnly) = ServerToolResults(only = only)
             ...
             */
            @JvmStatic
            fun ofExcept(except: WebFetchUrlSourceExcept) = ServerToolResults(except = except)
        ...
        internal class Deserializer :
            ...
            override fun ObjectCodec.deserialize(node: JsonNode): ServerToolResults {
                ...
                when (type) {
                    "all" -> {
                        return tryDeserialize(node, jacksonTypeRef<WebFetchUrlSourceAll>())?.let {
                            ServerToolResults(all = it, _json = json)
                        } ?: ServerToolResults(_json = json)
                    }
                    "none" -> {
                        return tryDeserialize(node, jacksonTypeRef<WebFetchUrlSourceNone>())?.let {
                            ServerToolResults(none = it, _json = json)
                        } ?: ServerToolResults(_json = json)
                    }
                    "only" -> {
                        return tryDeserialize(node, jacksonTypeRef<WebFetchUrlSourceOnly>())?.let {
                            ServerToolResults(only = it, _json = json)
                        } ?: ServerToolResults(_json = json)
                    }
                    "except" -> {
                        return tryDeserialize(node, jacksonTypeRef<WebFetchUrlSourceExcept>())
    ...
    private constructor(
        ...
        companion object {
             ...
             * the tool filter's source, or all user input.
             */
            @JvmStatic fun ofAll(all: WebFetchUrlSourceAll) = UserInput(all = all)

            /**
             * The ``url_sources`` variant under which a source contributes nothing: no result of
             * the tool filter's source, or no user input.
             */
            @JvmStatic fun ofNone(none: WebFetchUrlSourceNone) = UserInput(none = none)
        }
        ...
        internal class Deserializer : BaseDeserializer<UserInput>(UserInput::class) {
            ...
            override fun ObjectCodec.deserialize(node: JsonNode): UserInput {
                ...
                when (type) {
                    "all" -> {
                        return tryDeserialize(node, jacksonTypeRef<WebFetchUrlSourceAll>())?.let {
                            UserInput(all = it, _json = json)
                        } ?: UserInput(_json = json)
                    }
                    "none" -> {
                        return tryDeserialize(node, jacksonTypeRef<WebFetchUrlSourceNone>())?.let {
                            UserInput(none = it, _json = json)
                        } ?: UserInput(_json = json)
                    }
                }
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return other is WebFetchUrlSources &&
    ...
    override fun toString() =
        "WebFetchUrlSources{clientToolResults=$clientToolResults, serverToolResults=$serverToolResults, userInput=$userInput, additionalProperties=$additionalProperties}"
}

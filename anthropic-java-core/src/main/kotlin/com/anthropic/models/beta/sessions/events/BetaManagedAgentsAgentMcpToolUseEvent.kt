...
private constructor(
     ...
     * the server routes the matching `user.tool_confirmation` by `tool_use_id`, so clients do not
     * send it back.
     *
     * @throws AnthropicInvalidDataException if the JSON field has an unexpected type (e.g. if the
     *   server responded with an unexpected value).
    ...
    class Builder internal constructor() {
        ...
        internal fun from(
            ...
            evaluation = betaManagedAgentsAgentMcpToolUseEvent.evaluation
         ...
         * only: the server routes the matching `user.tool_confirmation` by `tool_use_id`, so
    ...
    override fun toString() =
        "BetaManagedAgentsAgentMcpToolUseEvent{id=$id, input=$input, mcpServerName=$mcpServerName, name=$name, processedAt=$processedAt, type=$type, evaluatedPermission=$evaluatedPermission, evaluation=$evaluation, sessionThreadId=$sessionThreadId, additionalProperties=$additionalProperties}"
}

 ...
 *
 * `model` is required. The four override fields (`max_tokens`, `thinking`, `output_config`, and
 * `speed`) replace the corresponding top-level field for this attempt only and are validated as if
 * the request were made to `model`. Any other key is rejected at parse time.
 */

 ...
 *
 * A service account is a named workload identity that federation rules target. `organization_role`
 * is `developer` (default) or `admin`; a rule may only be created or retargeted to grant
 * `org:admin` scope when the target's `organization_role` is `admin`. Creating an `admin`-role
 * service account requires an interactive credential (a user OAuth token or a Console session) — a
...
private constructor(
    ...
    class Builder internal constructor() {
        ...
        internal fun from(serviceAccountCreateParams: ServiceAccountCreateParams) = apply {
            betas = serviceAccountCreateParams.betas?.toMutableList()
    ...
    private constructor(
        ...
        ) : this(name, description, organizationRole, mutableMapOf())
        ...
        private val hashCode: Int by lazy {
            Objects.hash(name, description, organizationRole, additionalProperties)
        }

        override fun hashCode(): Int = hashCode

        override fun toString() =
            "Body{name=$name, description=$description, organizationRole=$organizationRole, additionalProperties=$additionalProperties}"
    }
    ...
    override fun toString() =
        "ServiceAccountCreateParams{betas=$betas, body=$body, additionalHeaders=$additionalHeaders, additionalQueryParams=$additionalQueryParams}"
}

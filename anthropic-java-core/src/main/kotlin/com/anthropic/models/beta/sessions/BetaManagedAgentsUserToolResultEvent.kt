...
import com.anthropic.models.beta.sessions.events.BetaManagedAgentsSearchResultBlock
 ...
 * `self_hosted` environments, where sandbox-routed tools are executed by the client rather than the
 * server.
 */
...
private constructor(
    ...
    ) : this(id, toolUseId, type, content, isError, processedAt, sessionThreadId, mutableMapOf())
    ...
    /**
     * The id of the `agent.tool_use` event this result corresponds to, which can be found in the
     * last `session.status_idle`
    ...
    class Builder internal constructor() {
        ...
        internal fun from(
            ...
            id = betaManagedAgentsUserToolResultEvent.id
            toolUseId = betaManagedAgentsUserToolResultEvent.toolUseId
            type = betaManagedAgentsUserToolResultEvent.type
            content = betaManagedAgentsUserToolResultEvent.content.map { it.toMutableList() }
            isError = betaManagedAgentsUserToolResultEvent.isError
            processedAt = betaManagedAgentsUserToolResultEvent.processedAt
            sessionThreadId = betaManagedAgentsUserToolResultEvent.sessionThreadId
            additionalProperties =
                betaManagedAgentsUserToolResultEvent.additionalProperties.toMutableMap()
        }
        ...
        /**
         * The id of the `agent.tool_use` event this result corresponds to, which can be found in
         ...
         */
        fun build(): BetaManagedAgentsUserToolResultEvent =
            BetaManagedAgentsUserToolResultEvent(
                checkRequired("id", id),
     ...
     *   expected type.
     */
    fun validate(): BetaManagedAgentsUserToolResultEvent = apply {
        if (validated) {
            return@apply
        }
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return other is BetaManagedAgentsUserToolResultEvent &&
            id == other.id &&
    ...
    override fun toString() =
        "BetaManagedAgentsUserToolResultEvent{id=$id, toolUseId=$toolUseId, type=$type, content=$content, isError=$isError, processedAt=$processedAt, sessionThreadId=$sessionThreadId, additionalProperties=$additionalProperties}"
}

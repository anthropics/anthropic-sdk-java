...
private constructor(
    ...
    class Builder internal constructor() {
        ...
        /**
         * Alias for calling [contentBlock] with
         * `ContentBlock.ofAdvisorToolResult(advisorToolResult)`.
         */
        fun contentBlock(advisorToolResult: BetaAdvisorToolResultBlock) =
            contentBlock(ContentBlock.ofAdvisorToolResult(advisorToolResult))
        ...
        /** Alias for calling [contentBlock] with `ContentBlock.ofFallback(fallback)`. */
        fun contentBlock(fallback: BetaFallbackBlock) =
            contentBlock(ContentBlock.ofFallback(fallback))

        /**
         * Alias for calling [contentBlock] with `ContentBlock.ofMcpToolListing(mcpToolListing)`.
         */
        fun contentBlock(mcpToolListing: BetaMcpToolListingBlock) =
            contentBlock(ContentBlock.ofMcpToolListing(mcpToolListing))
    ...
    private constructor(
         ...
         */
        fun mcpToolListing(): Optional<BetaMcpToolListingBlock> =
         ...
         * this listing instead of asking the MCP server again.
         */
        fun asMcpToolListing(): BetaMcpToolListingBlock =
        ...
        override fun toString(): String =
            when {
                ...
                advisorToolResult != null -> "ContentBlock{advisorToolResult=$advisorToolResult}"
                codeExecutionToolResult != null ->
                ...
                fallback != null -> "ContentBlock{fallback=$fallback}"
                mcpToolListing != null -> "ContentBlock{mcpToolListing=$mcpToolListing}"
        ...
        companion object {
            ...
            fun ofAdvisorToolResult(advisorToolResult: BetaAdvisorToolResultBlock) =
                ContentBlock(advisorToolResult = advisorToolResult)
             ...
             * declined. A turn where no preceding model ran and declined has no such boundary and
             * carries no block — the signal for whether a fallback model served the response is the
             * presence of a `fallback_message` entry in `usage.iterations`, not this block.
             *
             * The block is treated like a server-tool content block for streaming: it arrives via
             * the standard `content_block_start` / `content_block_stop` pair and carries no deltas.
             */
            @JvmStatic
            fun ofFallback(fallback: BetaFallbackBlock) = ContentBlock(fallback = fallback)
            ...
            fun ofMcpToolListing(mcpToolListing: BetaMcpToolListingBlock) =
                ContentBlock(mcpToolListing = mcpToolListing)
        }
        ...
        internal class Deserializer : BaseDeserializer<ContentBlock>(ContentBlock::class) {
            ...
            override fun ObjectCodec.deserialize(node: JsonNode): ContentBlock {
                ...
                when (type) {
                    ...
                    "advisor_tool_result" -> {
                        return tryDeserialize(node, jacksonTypeRef<BetaAdvisorToolResultBlock>())
                            ?.let { ContentBlock(advisorToolResult = it, _json = json) }
                            ?: ContentBlock(_json = json)
                    }
                    ...
                    "fallback" -> {
                        return tryDeserialize(node, jacksonTypeRef<BetaFallbackBlock>())?.let {
                            ContentBlock(fallback = it, _json = json)
                        } ?: ContentBlock(_json = json)
                    }
                    "mcp_tool_listing" -> {
                        return tryDeserialize(node, jacksonTypeRef<BetaMcpToolListingBlock>())
                            ?.let { ContentBlock(mcpToolListing = it, _json = json) }
                            ?: ContentBlock(_json = json)
                    }
                }
        ...
        internal class Serializer : BaseSerializer<ContentBlock>(ContentBlock::class) {
            ...
            override fun serialize(
                ...
                when {
                    ...
                    value.advisorToolResult != null ->
                        generator.writeObject(value.advisorToolResult)

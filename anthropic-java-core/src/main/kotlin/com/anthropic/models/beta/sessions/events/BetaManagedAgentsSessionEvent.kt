...
@JsonDeserialize(using = BetaManagedAgentsSessionEvent.Deserializer::class)
@JsonSerialize(using = BetaManagedAgentsSessionEvent.Serializer::class)
class BetaManagedAgentsSessionEvent
private constructor(
     ...
     * import java.util.Optional;
     *
     * Optional<String> result = betaManagedAgentsSessionEvent.accept(new BetaManagedAgentsSessionEvent.Visitor<Optional<String>>() {
     *     @Override
     ...
     *   expected type.
     */
    fun validate(): BetaManagedAgentsSessionEvent = apply {
        if (validated) {
            return@apply
        }
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return other is BetaManagedAgentsSessionEvent &&
    ...
    override fun toString(): String =
        when {
            userMessage != null -> "BetaManagedAgentsSessionEvent{userMessage=$userMessage}"
            userInterrupt != null -> "BetaManagedAgentsSessionEvent{userInterrupt=$userInterrupt}"
            userToolConfirmation != null ->
                "BetaManagedAgentsSessionEvent{userToolConfirmation=$userToolConfirmation}"
            userCustomToolResult != null ->
                "BetaManagedAgentsSessionEvent{userCustomToolResult=$userCustomToolResult}"
            agentCustomToolUse != null ->
                "BetaManagedAgentsSessionEvent{agentCustomToolUse=$agentCustomToolUse}"
            agentMessage != null -> "BetaManagedAgentsSessionEvent{agentMessage=$agentMessage}"
            agentThinking != null -> "BetaManagedAgentsSessionEvent{agentThinking=$agentThinking}"
            agentMcpToolUse != null ->
                "BetaManagedAgentsSessionEvent{agentMcpToolUse=$agentMcpToolUse}"
            agentMcpToolResult != null ->
                "BetaManagedAgentsSessionEvent{agentMcpToolResult=$agentMcpToolResult}"
            ...
            sessionStatusRescheduled != null ->
                "BetaManagedAgentsSessionEvent{sessionStatusRescheduled=$sessionStatusRescheduled}"
            sessionStatusRunning != null ->
                "BetaManagedAgentsSessionEvent{sessionStatusRunning=$sessionStatusRunning}"
            ...
            userToolResult != null ->
                "BetaManagedAgentsSessionEvent{userToolResult=$userToolResult}"
            sessionThreadStatusRescheduled != null ->
    ...
    companion object {
        ...
        fun ofUserMessage(userMessage: BetaManagedAgentsUserMessageEvent) =
            BetaManagedAgentsSessionEvent(userMessage = userMessage)
        ...
        fun ofAgentMessage(agentMessage: BetaManagedAgentsAgentMessageEvent) =
            BetaManagedAgentsSessionEvent(agentMessage = agentMessage)
        ...
        fun ofAgentThinking(agentThinking: BetaManagedAgentsAgentThinkingEvent) =
            BetaManagedAgentsSessionEvent(agentThinking = agentThinking)
        ...
        fun ofAgentToolUse(agentToolUse: BetaManagedAgentsAgentToolUseEvent) =
            BetaManagedAgentsSessionEvent(agentToolUse = agentToolUse)
        ...
        fun ofSessionError(sessionError: BetaManagedAgentsSessionErrorEvent) =
            BetaManagedAgentsSessionEvent(sessionError = sessionError)
        ...
        fun ofSystemMessage(systemMessage: BetaManagedAgentsSystemMessageEvent) =
            BetaManagedAgentsSessionEvent(systemMessage = systemMessage)
    }

    /**
     * An interface that defines how to map each variant of [BetaManagedAgentsSessionEvent] to a
     * value of type [T].
     */
    interface Visitor<out T> {
         ...
         *
         * An instance of [BetaManagedAgentsSessionEvent] can contain an unknown variant if it was
        ...
        fun unknown(json: JsonValue?): T {
            throw AnthropicInvalidDataException("Unknown BetaManagedAgentsSessionEvent: $json")
        }
    }

    internal class Deserializer :
        BaseDeserializer<BetaManagedAgentsSessionEvent>(BetaManagedAgentsSessionEvent::class) {

        override fun ObjectCodec.deserialize(node: JsonNode): BetaManagedAgentsSessionEvent {
            ...
            when (type) {
                "user.message" -> {
                    return tryDeserialize(node, jacksonTypeRef<BetaManagedAgentsUserMessageEvent>())
                        ?.let { BetaManagedAgentsSessionEvent(userMessage = it, _json = json) }
                ...
                "user.interrupt" -> {
                    return tryDeserialize(
                        ...
                        ?.let { BetaManagedAgentsSessionEvent(userInterrupt = it, _json = json) }
                        ?: BetaManagedAgentsSessionEvent(_json = json)
                }
                "user.tool_confirmation" -> {
                    return tryDeserialize(
                        ...
                        ?.let {
                            BetaManagedAgentsSessionEvent(userToolConfirmation = it, _json = json)
                        } ?: BetaManagedAgentsSessionEvent(_json = json)
                }
                "user.custom_tool_result" -> {
                    return tryDeserialize(
                        ...
                        ?.let {
                            BetaManagedAgentsSessionEvent(userCustomToolResult = it, _json = json)
                        } ?: BetaManagedAgentsSessionEvent(_json = json)
                }
                "agent.custom_tool_use" -> {
                    return tryDeserialize(
                        ...
                        ?.let {
                            BetaManagedAgentsSessionEvent(agentCustomToolUse = it, _json = json)
                        } ?: BetaManagedAgentsSessionEvent(_json = json)
                }
                "agent.message" -> {
                    return tryDeserialize(
                        ...
                        ?.let { BetaManagedAgentsSessionEvent(agentMessage = it, _json = json) }
                        ?: BetaManagedAgentsSessionEvent(_json = json)
                }
                "agent.thinking" -> {
                    return tryDeserialize(
                        ...
                        ?.let { BetaManagedAgentsSessionEvent(agentThinking = it, _json = json) }
                        ?: BetaManagedAgentsSessionEvent(_json = json)
                }
                "agent.mcp_tool_use" -> {
                    return tryDeserialize(
                        ...
                        ?.let { BetaManagedAgentsSessionEvent(agentMcpToolUse = it, _json = json) }
                        ?: BetaManagedAgentsSessionEvent(_json = json)
                }
                "agent.mcp_tool_result" -> {
                    return tryDeserialize(
                        ...
                        ?.let {
                            BetaManagedAgentsSessionEvent(agentMcpToolResult = it, _json = json)
                        } ?: BetaManagedAgentsSessionEvent(_json = json)
                }
                "agent.tool_use" -> {
                    return tryDeserialize(
                        ...
                        ?.let { BetaManagedAgentsSessionEvent(agentToolUse = it, _json = json) }
                        ?: BetaManagedAgentsSessionEvent(_json = json)
                }
                "agent.tool_result" -> {
                    return tryDeserialize(
                ...
                "session.error" -> {
                    return tryDeserialize(
                        ...
                        ?.let { BetaManagedAgentsSessionEvent(sessionError = it, _json = json) }
                        ?: BetaManagedAgentsSessionEvent(_json = json)
                }
                ...
                "session.status_running" -> {
                    return tryDeserialize(
                        ...
                        ?.let {
                            BetaManagedAgentsSessionEvent(sessionStatusRunning = it, _json = json)
                        } ?: BetaManagedAgentsSessionEvent(_json = json)
                }
                "session.status_idle" -> {
                    return tryDeserialize(
                        ...
                        ?.let {
                            BetaManagedAgentsSessionEvent(sessionStatusIdle = it, _json = json)
                        } ?: BetaManagedAgentsSessionEvent(_json = json)
                }
                ...
                "span.model_request_start" -> {
                    return tryDeserialize(
                        ...
                        ?.let {
                            BetaManagedAgentsSessionEvent(spanModelRequestStart = it, _json = json)
                        } ?: BetaManagedAgentsSessionEvent(_json = json)
                }
                ...
                "user.tool_result" -> {
                    return tryDeserialize(
                        ...
                        ?.let { BetaManagedAgentsSessionEvent(userToolResult = it, _json = json) }
                        ?: BetaManagedAgentsSessionEvent(_json = json)
                }
                ...
                "session.updated" -> {
                    return tryDeserialize(
                        ...
                        ?.let { BetaManagedAgentsSessionEvent(sessionUpdated = it, _json = json) }
                        ?: BetaManagedAgentsSessionEvent(_json = json)
                }
                "system.message" -> {
                    return tryDeserialize(
                        ...
                        ?.let { BetaManagedAgentsSessionEvent(systemMessage = it, _json = json) }
                        ?: BetaManagedAgentsSessionEvent(_json = json)
                }
            }
    ...
    internal class Serializer :
        BaseSerializer<BetaManagedAgentsSessionEvent>(BetaManagedAgentsSessionEvent::class) {

        override fun serialize(
            value: BetaManagedAgentsSessionEvent,

...
private constructor(
    ...
    /**
     * Only fetch URLs whose host is one of these domains or a subdomain of one. Each entry is a
     * plain hostname like "docs.example.com" (no scheme, port, or path). At most 64 entries; an
     * empty list is rejected (omit the field instead). Cannot be combined with blocked_domains.
     *
    ...
    /**
     * Never fetch URLs whose host is one of these domains or a subdomain of one. Each entry is a
     * plain hostname like "ads.example.com" (no scheme, port, or path). At most 64 entries; an
     * empty list is rejected (omit the field instead). Cannot be combined with allowed_domains.
     *
     * @throws AnthropicInvalidDataException if the JSON field has an unexpected type (e.g. if the
     *   server responded with an unexpected value).
     */
    ...
    /**
     * Maximum number of tokens of fetched text content to include in context per call. Does not
     * apply to binary content such as PDFs.
     *
     * @throws AnthropicInvalidDataException if the JSON field has an unexpected type (e.g. if the
     *   server responded with an unexpected value).
     */
    ...
    /** A builder for [BetaManagedAgentsWebFetchToolConfigParams]. */
    class Builder internal constructor() {
        ...
        internal fun from(
            ...
            name = betaManagedAgentsWebFetchToolConfigParams.name
            allowedDomains =
                betaManagedAgentsWebFetchToolConfigParams.allowedDomains
                    .map { it.toMutableList() }
                    .takeUnless { it.isMissing() }
            blockedDomains =
                betaManagedAgentsWebFetchToolConfigParams.blockedDomains
                    .map { it.toMutableList() }
                    .takeUnless { it.isMissing() }
            enabled = betaManagedAgentsWebFetchToolConfigParams.enabled
        ...
        /**
         * Never fetch URLs whose host is one of these domains or a subdomain of one. Each entry is
         * a plain hostname like "ads.example.com" (no scheme, port, or path). At most 64 entries;
         * an empty list is rejected (omit the field instead). Cannot be combined with
         * allowed_domains.
         */
        ...
        /**
         * Returns an immutable instance of [BetaManagedAgentsWebFetchToolConfigParams].
         *
         * Further updates to this [Builder] will not mutate the returned instance.
         */
        fun build(): BetaManagedAgentsWebFetchToolConfigParams =
            BetaManagedAgentsWebFetchToolConfigParams(
                name,
     ...
     *   expected type.
     */
    fun validate(): BetaManagedAgentsWebFetchToolConfigParams = apply {
        if (validated) {
            return@apply
        }
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return other is BetaManagedAgentsWebFetchToolConfigParams &&
            name == other.name &&

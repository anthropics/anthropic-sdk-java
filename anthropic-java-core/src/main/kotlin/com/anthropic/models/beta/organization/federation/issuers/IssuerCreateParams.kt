 ...
 * (fetch keys from a fixed JWKS URL), or `inline` (provide a static key set). When `jwks.type` is
 * `discovery` and no `discovery_base` is set, the issuer URL must be publicly reachable over HTTPS
 * so Anthropic can fetch the discovery document; for `explicit_url` and `inline` modes the issuer
 * URL is only matched as the JWT's `iss` claim and is not fetched.
 */
class IssuerCreateParams
private constructor(
     ...
     */
    fun issuerUrl(): String = body.issuerUrl()
     ...
     * this issuer. Defaults to true. Applies only to assertions carrying a `jti` claim; tokens
     ...
     * 49h). Defaults to 3600 (1h). Assertions must carry both `iat` and `exp`; a missing `iat` is
     * rejected.
     *
     * @throws AnthropicInvalidDataException if the JSON field has an unexpected type (e.g. if the
     *   server responded with an unexpected value).
    ...
    companion object {
        ...
        /**
         * Returns a mutable builder for constructing an instance of [IssuerCreateParams].
         *
    ...
    /** A builder for [IssuerCreateParams]. */
    class Builder internal constructor() {
        ...
        @JvmSynthetic
        internal fun from(issuerCreateParams: IssuerCreateParams) = apply {
            betas = issuerCreateParams.betas?.toMutableList()
            body = issuerCreateParams.body.toBuilder()
            additionalHeaders = issuerCreateParams.additionalHeaders.toBuilder()
            additionalQueryParams = issuerCreateParams.additionalQueryParams.toBuilder()
        }
         ...
         * from this issuer. Defaults to true. Applies only to assertions carrying a `jti` claim;
         ...
         * to 49h). Defaults to 3600 (1h). Assertions must carry both `iat` and `exp`; a missing
         * `iat` is rejected.
         */
        ...
        /**
         * Returns an immutable instance of [IssuerCreateParams].
         *
    ...
    private constructor(
        ...
        ) : this(issuerUrl, name, checkJti, jwks, maxJwtLifetimeSeconds, mutableMapOf())
        ...
        class Builder internal constructor() {
             ...
             * i.e. up to 49h). Defaults to 3600 (1h). Assertions must carry both `iat` and `exp`; a
        ...
        override fun toString() =
            "Body{issuerUrl=$issuerUrl, name=$name, checkJti=$checkJti, jwks=$jwks, maxJwtLifetimeSeconds=$maxJwtLifetimeSeconds, additionalProperties=$additionalProperties}"
    }
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return other is IssuerCreateParams &&
            betas == other.betas &&
            body == other.body &&
    ...
    override fun toString() =
        "IssuerCreateParams{betas=$betas, body=$body, additionalHeaders=$additionalHeaders, additionalQueryParams=$additionalQueryParams}"
}

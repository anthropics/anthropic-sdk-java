 ...
 * the Anthropic API specification for the input schemas used to define structured outputs. Only a
 ...
 * specification, but to the Anthropic API specification's restrictions on JSON schemas.
 */
internal class JsonSchemaValidator private constructor() {

    companion object {
        private const val NO_PROPERTIES_DOC =
            "https://github.com/anthropics/anthropic-sdk-java#defining-json-schema-properties"
        ...
        // schema to define its own "vocabulary" of type names. "integer" is supported by Anthropic.
         ...
         * present. Anthropic allows the `"anyOf"` field in sub-schemas, but not in the root schema.
         */
    ...
    /**
     * Validates a schema with respect to the Anthropic API specifications.
     *
    ...
    fun validate(rootSchema: JsonNode): JsonSchemaValidator {
        ...
        validateSchema(rootSchema, ROOT_PATH, depth = ROOT_DEPTH)
     ...
     * @param depth The current depth of nesting. The Anthropic API specification places a maximum
    ...
    /**
     * Validates a schema if it has an `"anyOf"` field. The value of the field is expected to be an
     * array of valid sub-schemas. If the schema has no `"anyOf"` field, no action is taken.
     */
     ...
     * sub-schema.
     *
     * The value of the `"minItems"` field, if present, must be within the allowed range.
     */
    private fun validateArraySchema(schema: JsonNode, path: String, depth: Int) {
        ...
        // `"items"` is not required in an array for Anthropic. However, if no `"items"` field is
        // present, the array's items have no `"type"` and there is then a restriction on the
        // nesting depth in the JSON document. There will be no nesting below this point in the
        // schema, as there is no type of content to recurse into.
        //
        // `"items"` may also be present but empty if the array corresponds to a type without a
        // generic subtype. For example, for a `List` field in Java. Only recurse if the field is
        // present and is not empty (it is expected to have a `"type"` field at a minimum if it
        // represents a valid sub-schema).
        //
        // If present, the `"items"` field must be an object (which may or may not be empty).
        val items = schema.get(ITEMS)

        verify(items == null || items.isObject, path) { "'$ITEMS' field is not an object." }
    ...
    override fun toString(): String =
        "${javaClass.simpleName}{isValidationComplete=$isValidationComplete, errors=$errors}"
}
